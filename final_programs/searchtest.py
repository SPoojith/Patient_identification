...
class SearchP:
    ...
    def outgui(root,Patient_firstName,Patient_lastName,gender,dateOfBirth,Patient_PhoneNumber,Patient_email,Patient_bloodGroup,Patient_aadhar,MedicationList,Emergency_firstName,Emergency_lastName,RelationToPatient,Emergency_ContactNumber,Emergency_ContactEmail,Pdf_filename,Pdf_data,Medical_history):
        ...
        img = Image.open(os.path.join(os.path.join(SearchP.path_all_images,Patient_aadhar+"/"+Patient_aadhar+"_0.jpg")))
        ...
        def mailto():
            ...
            html ="<html><body><p><h1>    You Received this mail because you were listed as an emergency contact person for <br>** "+Patient_firstName+" "+Patient_lastName+"Who is admited to **Vijayanagr hospital ** </h1><h2>    Contact us immeditly      <br>Hospital Contact Number:<br> 080 000 000 0000    <br>Hospital Address:<br> Vijayanagar Banglore-60</h2></p></body></html>"
    ...
    def findp(piname):
        ...
        app = SearchP.outgui(root,Patient_firstName,Patient_lastName,gender,dateOfBirth,Patient_PhoneNumber,Patient_email,Patient_bloodGroup,Patient_aadhar,MedicationList,Emergency_firstName,Emergency_lastName,RelationToPatient,Emergency_ContactNumber,Emergency_ContactEmail,Pdf_filename,Pdf_data,Medical_history)
    
    def flow1():
        tot = tk.Tk()
        tot.title(" collect Patients details")
        tot.geometry("600x300")
        tot.resizable(False,False)
        frame1=tk.Frame(tot,bg="BLACK")
        frame1.place(x=0,y=0,width=600,height=400)
        register_lbl=tk.Label(frame1,text="ENTER Aadhar :",font=("times new roman ",20,"bold"),fg="orange",bg="black")
        register_lbl.place(x=20,y=50)
        usn=tk.StringVar()
        f_name_entry=ttk.Entry(frame1,font=("times new roman ",15),textvariable=usn)
        f_name_entry.place(x=250,y=51,width=250)
        def qui():
            tot.quit()
            tot.destroy()
            SearchP.findp(usn.get())
        closeButton = tk.Button(frame1,text="Search",font=("times new roman ",15,"bold"),command=qui)
